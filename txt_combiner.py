...
def combine_files(files, num_files, output_dir):
    for i in range(0, len(files), num_files):
        ...
        with open(os.path.join(output_dir, f"{start//num_files+1}.txt"), 'w', encoding='utf-8')as outfile:
...
def select_files():
    ...
    file_paths = sorted(file_paths, key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))
    output_dir = filedialog.askdirectory(initialdir=os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))))
